...
기본 메인 실행 버튼입니다. CST를 열고 형상/경계조건/Floquet port를 만든 뒤 Frequency Solver Start를 실행하고, Touchstone `.s2p`를 export합니다.
...
기본 해석/스윕 실행에서는 Python이 CST에 파라미터를 먼저 등록한 뒤 `length-width` 같은 식을 그대로 넘깁니다. 그래서 CST에서 스윕값을 바꾸고 같은 Start를 반복할 수 있습니다.
