...
4. 2. 실행 전 확인
5. 3. CST에 형상 만들기
6. CST에서 Setup Solver의 Start 실행
```
...
파일을 남기는 버튼은 기본적으로 이 버튼과 `배치 스윕 실행 + 결과폴더`입니다.
...
기본 메인 실행 버튼입니다. CST를 열고 형상/경계조건만 만든 뒤 저장은 하지 않습니다.
...
주의: 이 기능은 CST 내부 Parametric Sweep이 아닙니다. 값마다 JSON을 새로 만들고 CST를 순차 실행해서, 값별 `runs` 폴더를 만드는 **Python 배치 실행**입니다. 나중에 Python 예측 모듈 결과와 비교하기 위한 데이터셋을 모으는 용도입니다.
