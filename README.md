...
기본 메인 실행 버튼입니다. CST를 열고 형상, `Background Normal`, x/y `unit cell`, z `open`, Floquet port를 만든 뒤 기본 `HF Time Domain` solver type으로 CST의 `Setup Solver -> Start`에 해당하는 Solver Start를 실행합니다. 해석이 끝나면 Touchstone `.s2p`를 export합니다.
...
- `Background Normal`, x/y `unit cell`, z `open`, Floquet mode `2`를 다시 적용합니다.
