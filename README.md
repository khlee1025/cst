...
기본 메인 실행 버튼입니다. CST를 열고 형상/경계조건을 만든 뒤 Frequency Solver Start를 실행하고, Touchstone `.s2p`를 export합니다.
...
값을 넣고 `스윕 실행 + 결과 보기`를 누르면 케이스별로 CST 해석을 돌리고 마지막에 S11/S21을 한 표로 합칩니다.
