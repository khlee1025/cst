...
2. `LLM 프롬프트 복사` 버튼을 누릅니다.
3. 로컬 LLM에 프롬프트와 요청 문장을 넣습니다.
4. 로컬 LLM이 만든 JSON을 오른쪽 `JSON 명령서` 칸에 붙여넣습니다.
5. `JSON 정렬`을 눌러 문법 오류가 없는지 봅니다.
6. `드라이런`을 눌러 CST에 보낼 명령을 미리 확인합니다.
7. CST가 있는 PC에서는 `CST 실행`을 누릅니다.
