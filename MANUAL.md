...
- 왼쪽 버튼: 실제 작업 순서
- 오른쪽: 실행 출력, S11/S21 결과, 스윕 설정, JSON 명령서 확인
...
왼쪽 요청 메모에는 자연어로 적어도 되고, 숫자를 직접 적어도 됩니다. 지금 화면은 초보자 흐름에 맞춰 `대사 적용` 버튼 하나로 시작합니다.
...
기본 solver로 돌리고 S11/S21 결과를 정리해줘.
```
...
오른쪽 `스윕 설정` 탭에서 파라미터 스윕을 실행합니다.

- 단일 변수: `width`, `length`, `thickness`, `fmin`, `fmax` 중 하나를 고르고 값 목록을 입력
- 전체 변수 조합: `전체 변수 조합`을 고르고 값표에 여러 줄 입력
...
이미 export한 `.s2p`, `.csv`, `.txt` 결과에서 S11/S21만 모읍니다.
...
대사에서 숫자를 잘 못 읽었을 때 직접 `length`, `width`, `thickness`, `fmin`, `fmax`를 고칩니다.
...
예를 들어 `실행 전 확인`에서 0이면 CST 없이도 JSON은 대체로 정상입니다. `CST 해석 + 결과 보기`에서 1 또는 2가 나오면 출력창 맨 아래의 원본 에러 메시지를 확인하면 됩니다.
...
그래도 실패하면 출력창 맨 아래의 원본 에러 메시지를 복사해서 확인하세요.
...
`CST 해석 + 결과 보기` 또는 `스윕 실행 + 결과 보기`는 결과를 남겨야 하므로 `runs` 아래에 실행 폴더를 만듭니다. `실행 전 확인`은 폴더를 만들지 않습니다.
...
`결과 불러오기`를 쓰면 선택한 폴더에 아래 파일이 생깁니다.
...
```text
S11/S21 결과 탭 확인 -> s11_s21_summary.csv를 Python 비교 모듈 입력으로 사용
```
