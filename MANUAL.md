...
2. `1. 대사 적용` 클릭
3. `2. 실행 전 확인` 클릭
4. 문제가 없으면 `3. CST에 형상 만들기`
5. CST에서 `Setup Solver`의 `Start` 실행
...
4. `실행 전 확인`
5. `CST에 형상 만들기`
6. CST에서 `Setup Solver`의 `Start` 실행
...
결과 비교용으로 실제 CST 실행과 결과 폴더 생성을 같이 합니다.

결과는 `runs` 폴더에 생깁니다.

파일을 남기는 버튼은 기본적으로 `CST 실행 + 결과폴더`와 `배치 스윕 실행 + 결과폴더`입니다.

### CST에 형상 만들기

메인 3단계 버튼입니다. CST를 열고 형상/경계조건만 만든 뒤 저장은 하지 않습니다.
...
주의: 이 기능은 CST 내부 Parametric Sweep이 아닙니다. 값마다 JSON을 새로 만들고 CST를 순차 실행해서, 값별 `runs` 폴더를 만드는 Python 배치 실행입니다.

사용 순서:

```text
1. 배치 스윕 클릭
2. 스윕 파라미터 입력 또는 선택, 예: width 또는 전체 변수 조합
3. 값 목록 입력, 예: 5, 10, 15
4. 배치 스윕 드라이런 또는 배치 스윕 실행 + 결과폴더 클릭
```

전체 변수 조합을 고르면 아래 값표를 사용합니다.
...
단, `배치 스윕 드라이런`은 미리보기/검증용이라 `runs` 폴더를 만들지 않습니다.
...
```text
대사 적용 -> 실행 전 확인 -> CST에 형상 만들기 -> CST Setup Solver Start
```

LLM 대사를 실험할 때:

```text
대사 -> JSON -> 실행 전 확인 -> CST에 형상 만들기
```
